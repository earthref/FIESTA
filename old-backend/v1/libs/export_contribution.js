...
class Exporter extends Runner {
	...
	toExcel(jsonToExport) {
		...
		for (let modelTable in this.model.tables) {
			...
			let tableHeaders = this.createExtendedHeadersData(
				modelTable,
				jsonToExport
			);
			let wsData = [
			...
			if (modelTable.toLowerCase() === 'measurements') {
				for (let columnName of tableHeaders.columnNameHeader) {
					columnOrderIdxs[columnName] = _.indexOf(
						jsonToExport[modelTable].columns,
						columnName
					);
				}
			}
			...
			for (
				let rowIdx = 0;
				rowIdx <
				(jsonToExport[modelTable].rows || jsonToExport[modelTable] || [])
					.length;
				rowIdx++
			) {
				...
				for (let columnName of tableHeaders.columnNameHeader) {
					...
					if (modelTable.toLowerCase() === 'measurements')
						value =
							jsonToExport[modelTable].rows[rowIdx][
								columnOrderIdxs[columnName]
							];
					else value = jsonToExport[modelTable][rowIdx][columnName];
					wsRow.push(value);
				}
          ...
          break;
			...
			for (let columnIdx = 0; columnIdx < wsData[rowIdx].length; columnIdx++) {
				...
				let cellStyle = {
					font: { bold: 'true' },
					alignment: {
						horizontal: columnIdx == 0 ? 'right' : 'center',
						vertical: 'center',
					},
					fill: { fgColor: { rgb: 'CCCCCC' } },
					border: {
						top: { style: 'thick', color: { rgb: '555555' } },
				...
				if (
					columnIdx > 0 &&
					columnIdx < wsData[rowIdx].length - 1 &&
					wsData[0][columnIdx + 1] != ''
				)
			...
			for (let rowIdx = 0; rowIdx < wsData.length; rowIdx++) {
				for (
					let columnIdx = 0;
					columnIdx < wsData[rowIdx].length;
					columnIdx++
				) {
					let cellAddress = XLSX.utils.encode_cell({ r: rowIdx, c: columnIdx });
					if (worksheet[cellAddress] !== undefined) {
						if (
							...
							(columnIdx > 0 ||
								worksheet[cellAddress].v.toString().slice(0, 8) !== 'Warning!')
						)
							columnWidths[columnIdx] = Math.max(
								columnWidths[columnIdx],
								worksheet[cellAddress].v.toString().length
							);
						if (worksheet[cellAddress].s === undefined)
							worksheet[cellAddress].s = {};
						if (worksheet[cellAddress].s.font === undefined)
	...
	_toSheet(data) {
		...
		for (var R = 0; R != data.length; ++R) {
			for (var C = 0; C != data[R].length; ++C) {
				...
				if (!isNaN(parseFloat(cell.v)) && isFinite(cell.v)) cell.t = 'n';
	...
	generateExtendedHeaderTSV(tableName, jsonToExport) {
		let extendedHeader = '';
		let extraHeaderData = this.createExtendedHeadersData(
			tableName,
			jsonToExport /*, orderedColumnArray*/
		);
		...
		extendedHeader = extendedHeader + '\n';
		extendedHeader =
			extendedHeader + extraHeaderData.columnTypeOrUnitHeader.join('\t');
		extendedHeader = extendedHeader + '\n';
		...
		for (let orderedColumnIdx in orderedColumnArray) {
			...
			if (tableName.toLowerCase() === 'measurements') {
				for (let jsonRowIdx in jsonToExport[tableName].columns) {
					...
					if (
						jsonToExport[tableName].columns[jsonRowIdx].toLowerCase() ===
							potentialColumnToAdd.toLowerCase() &&
						!discoveredColumns[potentialColumnToAdd]
					) {
						//and we haven't seen this column before
						...
						columnTypeOrUnitHeaderArray.push(
							this.getColumnTypeOrUnitString(tableName, potentialColumnToAdd)
						);
					}
				}
			} else {
				for (let jsonRowIdx in jsonToExport[tableName]) {
					if (
						jsonToExport[tableName][jsonRowIdx][potentialColumnToAdd] && //if this ordered column is found in the json file
						!discoveredColumns[potentialColumnToAdd]
					) {
						//and we haven't seen this column before
	...
	getColumnTypeOrUnitString(table, column) {
		...
		if (columnType === 'Number') {
			...
			if (
				columnUnit !== 'Dimensionless' &&
				columnUnit !== 'Custom' &&
				columnUnit !== ''
			)
				columnTypeOrUnitString += ` in ${columnUnit}`;
		} else if (columnUnit === 'Flag') columnTypeOrUnitString = 'Flag';
		else columnTypeOrUnitString = columnType;
	...
	createTSVfile(jsonToExport, toExtendedText) {
		...
		let text = ``;
		let numberOfTablesInJson = _.reduce(
			_.keys(jsonToExport),
			(n, key) => {
				if (key.slice(0, 1) !== '_') n += 1;
				return n;
			},
			0
		);
		...
		for (let orderedTableIdx in this.orderedModel) {
			let tableName = Object.getOwnPropertyNames(
				this.orderedModel[orderedTableIdx]
			)[0]; //loop through the tables in the model
			if (tableName && jsonToExport[tableName]) {
				//if the current table exists in the json to translate, add it to the output file
				text += `tab delimited\t${tableName}`;
				if (toExtendedText) text += `\t4 headers`;
				...
				if (toExtendedText) {
					...
					text += this.generateExtendedHeaderTSV(
						tableName,
						jsonToExport /*, orderedColumnArray*/
					);
				}
				...
				if (tableName.toLowerCase() === 'measurements') {
					...
					for (let orderedColIdx in this.orderedModel[orderedTableIdx][
						tableName
					]) {
						...
						for (let jsonRowsIdx in jsonToExport[tableName]) {
							...
							if (
								jsonToExport[tableName][jsonRowsIdx][
									orderedColumnNameToPotentiallyAdd
								] &&
								!columnsToAddToTSVheader[orderedColumnNameToPotentiallyAdd]
							) {
								columnsToAddToTSVheader[orderedColumnNameToPotentiallyAdd] =
									'have already seen this column';
								orderedListOfColumnsAddedToHeader.push(
									orderedColumnNameToPotentiallyAdd
								);
							}
						}
				...
				if (tableName.toLowerCase() === 'measurements') {
					...
					for (let jsonRowsIdx in jsonToExport[tableName]) {
						for (let colNameIdx in orderedListOfColumnsAddedToHeader) {
							...
							let dataToAdd =
								jsonToExport[tableName][jsonRowsIdx][columnName] || '';

							if (colNameIdx > 0 && colNameIdx < numberOfColumns)
								//no delimiter needed for the first column or at the end of the row
								text += '\t';

							text += this.handleSpecialCases(dataToAdd, tableName, columnName);

							if (colNameIdx == numberOfColumns - 1) text += '\n';
						}
					}
				}

				numberOfTablesInAddedToTSV++;
				if (numberOfTablesInAddedToTSV < numberOfTablesInJson)
					//no >>>> delimiter at end of file
					text += '>>>>>>>>>>\n';
			}
		}
		return text;
	...
	handleSpecialCases(value, tableName, columnName) {
		try {
			let type =
				models[_.last(versions)].tables[tableName].columns[columnName].type;

			if (type === 'Dictionary' && _.isPlainObject(value))
				value = _.keys(value)
					.map(
						(key) =>
							`${this.escapeColons(key)}[${this.escapeColons(value[key])}]`
					)
					.join(':');
			else if (type === 'Matrix' && _.isArray(value))
				value = value
					.map((row) => row.map((col) => this.escapeColons(col)).join(':'))
					.join(';');
	...
	testValidityOfTablesAndColumns(jsonToTranslate) {
		for (let newTableName in jsonToTranslate) {
			//this gets the string name of the property 'table'
				...
				for (let jsonColumnName of jsonToTranslate[newTableName].columns) {
					...
					if (!modelColumn || modelColumn == undefined) {
						if (!columnErrorReported[jsonColumnName]) {
							this._appendError(
								`column ${jsonColumnName} in table ${newTableName} is not defined in magic data model version ${this.version}`
							);
			...
			//Test to make sure the column names are valid for a given model
				...
				for (let jsonRowIdx in jsonToTranslate[newTableName]) {
					let jsonColumnNames = Object.getOwnPropertyNames(
   ...
   */
	createOrderedModel() {
		...
		for (let tableIdx in tableNames) {
			let properTablePositionIdx =
				this.model.tables[tableNames[tableIdx]].position - 1;
			orderedModel[properTablePositionIdx] = { [tableNames[tableIdx]]: [] };

			let columnPositionMap = [];
			let columnNames = Object.getOwnPropertyNames(
				this.model.tables[tableNames[tableIdx]].columns
			);
