...
class Summarizer extends Runner {
	...
	preSummarizePromise(contribution, meta) {
		...
		this._initProp(
			...
			'contribution',
			_.merge(
				{ summary: { contribution: { data_model_version: _.last(versions) } } },
				meta
			)
		);
	...
	/*_getCrossRefData() {
		...
		return new Promise((resolve) => {
			try {
				let doi = _.toUpper(
					_.trim(this.json.contribution.summary.contribution.reference)
				);
				if (doi === '') resolve();
				else
					...
					if (table === 'contribution') {
						...
						this.contribution.measurements.rows.forEach((row) => {
							if (row[experimentColumnIdx]) {
								...
								if (
									this.json.experiments[prop][parentProp].rows.length <
									maxExperimentMeasurementRows
								)
									this.json.experiments[prop][parentProp].rows.push(row);
								if (
									!onlyCounts &&
									this.json.experiments[prop][parentProp].summary.experiments
										._n_measurements < maxSummaryMeasurementRows
								)
								...
								this.json.experiments[prop][
									parentProp
								].summary.experiments._n_measurements += 1;
							}
						});
						...
						this.contribution[table].forEach((row) => {
							...
							if (name !== '') {
								this._initProp(
									this.json[joinTable][prop][parentProp],
									table,
									[]
								);
								this._initProp(
									this.json[joinTable][prop][parentProp].summary,
									table,
									{ ['_n_' + table]: 0 }
								);
						...
						this.contribution.criteria.forEach((row, criteriaRowIdx) => {
							...
							let criterionTable = criterionTableColumn.substr(
								0,
								criterionTableColumn.indexOf('.')
							);
							let criterionColumn = criterionTableColumn.substr(
								criterionTableColumn.indexOf('.') + 1
							);
							if (
								_.trim(row.criterion) !== '' &&
								_.indexOf(
									_.keys(models[_.last(versions)].tables),
									criterionTable.toLowerCase()
								) >= 0
							) {
								this._initProp(this.json.contribution, 'criteria', {
									rows: [],
								});
								this.json.contribution.criteria.rows.push(
									_.merge(
										{
								...
								this._initProp(this.json.contribution.summary, 'criteria', {
									_n_criteria: 0,
								});
								if (!onlyCounts)
									this._summarizeRowProps(
										row,
										this.json.contribution.summary.criteria,
										model
									);
								this.json.contribution.summary.criteria._n_criteria += 1;
								_.keys(this.json[criterionTable]).forEach((prop) => {
									_.keys(this.json[criterionTable][prop]).forEach(
										(parentProp) => {
											this._initProp(
												this.json[criterionTable][prop][parentProp],
												'criteria',
												{ rows: [] }
											);
											this.json[criterionTable][prop][
												parentProp
											].criteria.rows.push(
												_.merge(
													{
											...
											this._initProp(
												this.json[criterionTable][prop][parentProp].summary,
												'criteria',
												{ _n_criteria: 0 }
											);
											if (!onlyCounts)
											...
											this.json[criterionTable][prop][
												parentProp
											].summary.criteria._n_criteria += 1;
										}
									);
								});
							}
						...
						let parentColumn = sortedTables[tableIdx - 1].replace(
							reTrailingS,
							''
						);
						...
						this.contribution[table].forEach((row) => {
							...
							if (!onlyCounts)
								this._summarizeRowProps(
									row,
									this.json[table][prop][parentProp].summary[table],
									model
								);
				...
				if (table === 'contribution')
					...
					_.keys(this.json[table]).forEach((prop) => {
						_.keys(this.json[table][prop]).forEach((parentProp) => {
							this._consolidateSummary(
								this.json[table][prop][parentProp].summary
							);
						});
					});
				...
				if (table === 'measurements') {
					...
					_.keys(this.json[table]).forEach((tableProp) => {
						_.keys(this.json[table][tableProp]).forEach((parentProp) => {
							...
							if (this.json.specimens)
								_.keys(this.json.specimens[specimenProp]).forEach(
									(sampleProp) => {
										...
										if (this.json.samples)
											_.keys(this.json.samples[sampleProp]).forEach(
												(siteProp) => {
													this._aggregateSummaries(
														'samples',
													...
													if (this.json.sites)
														_.keys(this.json.sites[siteProp]).forEach(
															(locationProp) => {
																this._aggregateSummaries(
																	'sites',
																...
																if (
																	this.json.sites[siteProp][locationProp].ages
																) {
																	this._aggregateSummaries(
																		'ages',
																		this.json.sites[siteProp][locationProp],
																...
																if (
																	this.json.sites[siteProp][locationProp].images
																) {
																	this._aggregateSummaries(
																		'images',
																		this.json.sites[siteProp][locationProp],
																...
																if (this.json.locations)
																	_.keys(
																		this.json.locations[locationProp]
																	).forEach((contributionProp) => {
																		this._aggregateSummaries(
																			'locations',
																			this.json.locations[locationProp][
																				contributionProp
									...
									if (this.json.sites)
										_.keys(this.json.sites[siteProp]).forEach(
											(locationProp) => {
												...
												if (this.json.locations)
													_.keys(this.json.locations[locationProp]).forEach(
														(contributionProp) => {
															this._aggregateSummaries(
																'locations',
																this.json.locations[locationProp][
							...
							if (this.json.locations)
								_.keys(this.json.locations[locationProp]).forEach(
									(contributionProp) => {
										...
										if (
											this.json.locations[locationProp][contributionProp].ages
										) {
											this._aggregateSummaries(
												'ages',
												this.json.locations[locationProp][contributionProp],
										...
										if (
											this.json.locations[locationProp][contributionProp].images
										) {
											this._aggregateSummaries(
												'images',
												this.json.locations[locationProp][contributionProp],
			...
			return new Promise((resolve) => {
										...
										_.keys(this.json.sites[siteProp]).forEach(
											(locationProp) => {
												if (!toTable || toTable === 'sites') {
													...
													if (
														this.json[table][specimenProp][sampleProp].images
													) {
														this._aggregateSummaries(
															'images',
															this.json[table][specimenProp][sampleProp],
				...
				if (table === 'measurements') {
					_.keys(this.json.experiments).forEach((experimentProp) => {
						_.keys(this.json.experiments[experimentProp]).forEach(
							(specimenProp) => {
								if (!toTable || toTable === 'contribution') {
									this._aggregateSummaries(
										'experiments',
				...
				if (table === 'contribution') {
					...
					_.keys(this.json[table]).forEach((prop) => {
						_.keys(this.json[table][prop]).forEach((parentProp) => {
							...
							_.keys(this.json[table][prop][parentProp].summary).forEach(
								(summaryTable) => {
									if (
										summaryTable !== '_all' &&
										summaryTable !== 'contribution'
									) {
										this._aggregateSummaries(
											summaryTable,
	...
	_copyRowProps(row, summary, model) {
		...
		_.keys(row).forEach((column) => {
			if (model.columns[column] && model.columns[column].type === 'List') {
				...
				model.columns[column] &&
				model.columns[column].type === 'String'
			) {
				if (summary[column] === undefined)
				...
				this._appendError(
					`Unrecognized data model type "${model.columns[column].type}".`
				);
			} else {
	...
	_summarizeRowProps(row, summary, model) {
		...
		if (!isNaN(parseFloat(row.lon)) && !isNaN(parseFloat(row.lat))) {
			...
			if (!model.columns[column]) {
				...
				if (model.columns[column].type === 'List') {
					if (
						...
						_.isPlainObject(summary[column])
					) {
						if (row[column] && row[column].split)
							row[column]
								.split(':')
								.slice(0, maxValsArrayLength)
								.forEach((val) => {
									if (_.trim(val) !== '') {
										this._initProp(summary, column, {});
										summary[column][_.trim(val)] = true;
					...
					model.columns[column].type === 'String' ||
					model.columns[column].type === 'Matrix'
				) {
					if (_.trim(row[column]) !== '') {
						...
						if (_.keys(summary[column]).length < maxValsArrayLength) {
							let val;
							if (
								model.columns[column].unit === 'Flag' &&
								model.columns[column].validations
							) {
							...
							summary[column][
								val !== undefined ? val : _.trim(row[column])
							] = true;
						}
					}
	...
	_summerizeNumber(summary, column, val) {
		if (
			...
			val.n &&
			val.range &&
			val.range.gte !== undefined &&
			val.range.lte !== undefined
		) {
			this._initProp(summary, column, {
				vals: {},
				n: 0,
				range: { gte: undefined, lte: undefined },
			});
				...
				vals.forEach((v) => {
					if (_.isPlainObject(summary[column].vals)) {
						...
						if (_.keys(summary[column].vals).length >= maxValsArrayLength)
							summary[column].vals = _.keys(summary[column].vals).map((x) =>
								parseFloat(x)
							);
					}
				});
			...
			if (
				summary[column].range.gte === undefined ||
				summary[column].range.gte > val.range.gte
			) {
				summary[column].range.gte = val.range.gte;
			}
			if (
				summary[column].range.lte === undefined ||
			...
			if (column[0] !== '_' || reAgeSummaryColumn.test(column)) {
				...
				if (
					...
					if (
						val < 1e18 ||
						parseFloat(val).toExponential() === parseFloat(val).toString()
					)
						summary[column].range.gte = val;
				}
				if (
				...
				if (onlyCounts !== true) {
					if (model.columns[column]) {
						if (
							...
							this._summerizeNumber(
								to.summary[toName],
								column,
								from.summary[fromName][column]
							);
						} else {
							...
							if (_.isPlainObject(to.summary[toName][column])) {
								...
								vals.forEach((v) => {
									if (_.isPlainObject(to.summary[toName][column])) {
										...
										if (
											_.keys(to.summary[toName][column]).length >=
											maxValsArrayLength
										)
											to.summary[toName][column] = _.keys(
												to.summary[toName][column]
											);
									}
								});
							}
					...
					if (reGeoSummaryColumn.test(column)) {
						...
						if (to.summary[toName][column].length < maxValsArrayLength) {
							let geos = to.summary[toName][column].concat(
								from.summary[fromName][column]
							);
							to.summary[toName][column] = _.uniqBy(geos, JSON.stringify).slice(
								0,
								maxValsArrayLength
							);
						}
