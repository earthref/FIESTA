...
export default class extends Runner {
	...
	upgradePromise({
		json = undefined,
		nRowsBetweenProgressEvents = 1000,
		...
		if (!this.version) return Promise.resolve();
	...
	_mapPromise() {
		...
		const versionsToNewest = _.takeRightWhile(versions, (v) => {
			return v !== this.versionOld;
		});
		if (versionsToNewest.length === 0) return Promise.resolve();
		...
		return Promise.each(versionsToNewest, (versionNew, versionIdx) => {
			...
			const modelTables = _.orderBy(
				_.keys(this.modelOld['tables']),
				(t) => {
					return this.modelOld['tables'][t].position;
				},
				'asc'
			);
			const tables = _.union(
				_.intersection(modelTables, _.keys(this.jsonOld)),
				_.keys(this.jsonOld)
			);
			...
			if (this.versionNew === '3.0') {
				...
				this.modelOld['tables']['er_expeditions']['columns'][
					'er_location_name'
				] = {};
				this.modelNew['tables']['locations']['columns']['location'][
					'previous_columns'
				].push({ table: 'er_expeditions', column: 'er_location_name' });
				...
				if (this.jsonOld['er_specimens'] && this.jsonOld['er_synthetics']) {
					for (let jsonRowOldIdx in this.jsonOld['er_synthetics']) {
						...
						if (
							syntheticRow['er_specimen_name'] &&
							syntheticRow['er_synthetic_name']
						) {
							this.syntheticSpecimenNames[syntheticRow['er_specimen_name']] =
								syntheticRow['er_synthetic_name'];
						}
					}
				}
			...
			return Promise.each(tables, (jsonTableOld) => {
					...
					if (jsonTableOld === 'er_expeditions') {
						...
						for (let expeditionRowIdx in this.jsonOld['er_expeditions']) {
							...
							if (expeditionRow['expedition_location']) {
								...
								expeditionLocations = expeditionLocations.replace(
									/(^:|:$)/g,
									''
								);
						...
						this.jsonOld['pmag_results'].forEach((resultsRow) => {
							...
							if (
								...
								if (hasStratigraphic) {
									...
									if (
										stratigraphicRow['tilt_correction'] === undefined || // non stratigraphic correction may be provided
										hasTiltCorrected
									)
				...
				if (jsonTableOld === 'magic_measurements') {
					...
					if (
						!Array.isArray(this.jsonOld['magic_measurements'].columns) ||
						!Array.isArray(this.jsonOld['magic_measurements'].rows)
					) {
						this._appendError(
							`Table "${jsonTableOld}" ` + `has not been parsed properly.`
						);
					} else {
						// Create a measurements table.
						let newMeasurementsTableName =
							this.versionNew === '3.0' ? 'measurements' : 'magic_measurements';
						...
						for (let columnIdx in this.jsonOld['magic_measurements'].columns) {
							...
							if (
								!this.modelOld['tables'][jsonTableOld]['columns'][jsonColumnOld]
							) {
								if (
									!this.undefinedTableColumnErrors[
										jsonTableOld + '.' + jsonColumnOld
									]
								)
									this._appendError(
										`Column "${jsonColumnOld}" in table "${jsonTableOld}" ` +
							...
							if (
								(this.versionNew === '3.0' &&
									jsonColumnOld === 'measurement_time_zone') ||
								jsonColumnOld === 'er_synthetic_name'
							)
								pullIdxs.push(columnIdx);
							else
						...
						this.jsonNew[newMeasurementsTableName].rows = this.jsonOld[
							'magic_measurements'
						].rows.map((row) => {
							...
							if (this.versionNew === '3.0' && syntheticIdx >= 0) {
								if (specimenIdx === -1) row.push(row[syntheticIdx]);
							...
							if (
								this.versionNew === '3.0' &&
								(specimenIdx >= 0 || syntheticIdx >= 0)
							) {
								if (sampleIdx >= 0) {
									this.jsonNew.samples = this.jsonNew.samples || [];
									let samplesRowIdx =
										parentsIdx.samples['_' + row[sampleIdx]] ||
										_.findIndex(this.jsonNew.samples, {
											sample: row[sampleIdx],
										});
									if (samplesRowIdx === -1) {
										this.jsonNew.samples.push({ sample: row[sampleIdx] });
										parentsIdx.samples['_' + row[sampleIdx]] =
											this.jsonNew.samples.length - 1;
									}
								}
								...
								if (
									sampleIdx >= 0 &&
									specimensRowIdx >= 0 &&
									_.isEmpty(this.jsonNew.specimens[specimensRowIdx].sample)
								)
									this.jsonNew.specimens[specimensRowIdx].sample =
										row[sampleIdx];
							}
							if (this.versionNew === '3.0' && sampleIdx >= 0) {
								...
								if (
									siteIdx >= 0 &&
									samplesRowIdx >= 0 &&
									_.isEmpty(this.jsonNew.samples[samplesRowIdx].site)
								)
							...
							if (this.versionNew === '3.0' && siteIdx >= 0) {
								if (locationIdx >= 0) {
									...
									let locationsRowIdx =
										parentsIdx.locations['_' + row[locationIdx]] ||
										_.findIndex(this.jsonNew.locations, {
											location: row[locationIdx],
										});
									if (locationsRowIdx === -1) {
										this.jsonNew.locations.push({ location: row[locationIdx] });
										parentsIdx.locations['_' + row[locationIdx]] =
											this.jsonNew.locations.length - 1;
									}
								}
								...
								if (
									locationIdx >= 0 &&
									sitesRowIdx >= 0 &&
									_.isEmpty(this.jsonNew.sites[sitesRowIdx].location)
								)
							...
							if (this.versionNew === '3.0' && numberIdx >= 0) {
								if (
									isPositiveIntegerRe.test(row[numberIdx]) &&
									parseInt(row[numberIdx]) < 1000
								) {
									row.push('' + parseInt(row[numberIdx]));
									if (experimentIdx >= 0)
										row[numberIdx] = row[experimentIdx] + '-' + row[numberIdx];
								} else {
									row.push('');
								}
							}
					...
					rowProgressCounter += this.jsonOld[jsonTableOld].length;
					this.progress =
						versionIdx * versionPercentProgress +
						(versionPercentProgress * rowProgressCounter) / rowsTotal;
				...
				if (!this.modelOld['tables'][jsonTableOld]) {
					this._appendError(
						`Table "${jsonTableOld}" is not defined in MagIC Data Model version ${this.versionOld}.`
					);
				...
				const rowIdxChunks = _.chunk(
					_.range(this.jsonOld[jsonTableOld].length),
					this.nRowsBetweenProgressEvents
				);
				if (rowIdxChunks.length === 0) return Promise.resolve();
	...
	_mapTableRow(jsonTableOld, jsonRowOldIdx) {
		...
		if (this.versionNew === '3.0') {
			...
			if (jsonTableOld === 'pmag_results' || jsonTableOld === 'rmag_results') {
				if (
					jsonRowOld['er_synthetic_names'] != null &&
					!jsonRowOld['er_synthetic_names'].match(/.+:.+/)
				)
					joinTable = 'specimens';
				else if (
					jsonRowOld['er_specimen_names'] != null &&
					!jsonRowOld['er_specimen_names'].match(/.+:.+/)
				)
					joinTable = 'specimens';
				else if (
					jsonRowOld['er_sample_names'] != null &&
					!jsonRowOld['er_sample_names'].match(/.+:.+/)
				)
					joinTable = 'samples';
				else if (
					jsonRowOld['er_site_names'] != null &&
					!jsonRowOld['er_site_names'].match(/.+:.+/)
				)
					joinTable = 'sites';
				else if (
					jsonRowOld['er_location_names'] != null &&
					!jsonRowOld['er_location_names'].match(/.+:.+/)
				)
					joinTable = 'locations';
				else {
					this._appendWarning(
						`Row ${
							parseInt(jsonRowOldIdx) + 1
						} in table "${jsonTableOld}" was deleted in ` +
			...
			if (jsonRowOld['magic_method_codes']) {
				...
				let methodCodes = jsonRowOld['magic_method_codes']
					.replace(/(^:|:$)/g, '')
					.split(/:/);
				...
				let relativeIntensityNormalizations = _.intersection(methodCodes, [
					'IE-ARM',
					'IE-IRM',
					'IE-CHI',
				]);
				...
				jsonRowOld['magic_method_codes'] = _.without(
					methodCodes,
					'IE-ARM',
					'IE-IRM',
					'IE-CHI'
				...
				if (relativeIntensityNormalizations.length > 1)
					this._appendError(
						`Row ${
							parseInt(jsonRowOldIdx) + 1
						} in table "${jsonTableOld}" includes more than one ` +
			...
			if (jsonRowOld['location_geoid']) {
				if (!jsonRowOld['magic_method_codes'])
					jsonRowOld['magic_method_codes'] = '';
				jsonRowOld['magic_method_codes'] +=
					':GE-' + jsonRowOld['location_geoid'];
			}
			if (jsonRowOld['site_location_geoid']) {
				if (!jsonRowOld['magic_method_codes'])
					jsonRowOld['magic_method_codes'] = '';
				jsonRowOld['magic_method_codes'] +=
					':GE-' + jsonRowOld['site_location_geoid'];
			}
			if (jsonRowOld['sample_location_geoid']) {
				if (!jsonRowOld['magic_method_codes'])
					jsonRowOld['magic_method_codes'] = '';
				jsonRowOld['magic_method_codes'] +=
					':GE-' + jsonRowOld['sample_location_geoid'];
			}

			// Favor synthetic names over specimen names for natural synthetics.
			if (
				jsonTableOld === 'er_specimens' &&
				this.syntheticSpecimenNames[jsonRowOld['er_specimen_name']]
			) {
				if (jsonRowOld['er_specimen_alternatives']) {
					let alternatives = jsonRowOld['er_specimen_alternatives']
						.replace(/(^:|:$)/g, '')
						.split(/:/);
			...
			if (
				...
				if (jsonRowOld['eta_semi_angle'])
					poleConf[2] = jsonRowOld['eta_semi_angle'];
				if (jsonRowOld['zeta_dec']) poleConf[3] = jsonRowOld['zeta_dec'];
				if (jsonRowOld['zeta_inc']) poleConf[4] = jsonRowOld['zeta_inc'];
				if (jsonRowOld['zeta_semi_angle'])
					poleConf[5] = jsonRowOld['zeta_semi_angle'];
			...
			if (
				...
				let anisoS = ['', '', '', '', '', ''];
				if (jsonRowOld['anisotropy_s1'])
					anisoS[0] = jsonRowOld['anisotropy_s1'];
				if (jsonRowOld['anisotropy_s2'])
					anisoS[1] = jsonRowOld['anisotropy_s2'];
				if (jsonRowOld['anisotropy_s3'])
					anisoS[2] = jsonRowOld['anisotropy_s3'];
				if (jsonRowOld['anisotropy_s4'])
					anisoS[3] = jsonRowOld['anisotropy_s4'];
				if (jsonRowOld['anisotropy_s5'])
					anisoS[4] = jsonRowOld['anisotropy_s5'];
				if (jsonRowOld['anisotropy_s6'])
					anisoS[5] = jsonRowOld['anisotropy_s6'];
			...
			for (let i = 1; i <= 3; i++) {
				if (
					...
					let anisoV = ['', '', '', 'eta/zeta', '', '', '', '', '', ''];
					if (jsonRowOld['anisotropy_t' + i])
						anisoV[0] = jsonRowOld['anisotropy_t' + i];
					if (jsonRowOld['anisotropy_v' + i + '_dec'])
						anisoV[1] = jsonRowOld['anisotropy_v' + i + '_dec'];
					if (jsonRowOld['anisotropy_v' + i + '_inc'])
						anisoV[2] = jsonRowOld['anisotropy_v' + i + '_inc'];
					if (jsonRowOld['anisotropy_v' + i + '_eta_dec'])
						anisoV[4] = jsonRowOld['anisotropy_v' + i + '_eta_dec'];
					if (jsonRowOld['anisotropy_v' + i + '_eta_inc'])
						anisoV[5] = jsonRowOld['anisotropy_v' + i + '_eta_inc'];
					if (jsonRowOld['anisotropy_v' + i + '_eta_semi_angle'])
						anisoV[6] = jsonRowOld['anisotropy_v' + i + '_eta_semi_angle'];
					if (jsonRowOld['anisotropy_v' + i + '_zeta_dec'])
						anisoV[7] = jsonRowOld['anisotropy_v' + i + '_zeta_dec'];
					if (jsonRowOld['anisotropy_v' + i + '_zeta_inc'])
						anisoV[8] = jsonRowOld['anisotropy_v' + i + '_zeta_inc'];
					if (jsonRowOld['anisotropy_v' + i + '_zeta_semi_angle'])
						anisoV[9] = jsonRowOld['anisotropy_v' + i + '_zeta_semi_angle'];
			...
			// Convert dates into ISO 8601 timestamps.
			if (
				jsonRowOld['sample' + '_date'] ||
				jsonRowOld['sample' + '_time_zone'] ||
				jsonRowOld['image' + '_date'] ||
				jsonRowOld['image' + '_time_zone'] ||
				jsonRowOld['plot' + '_date'] ||
				jsonRowOld['plot' + '_time_zone']
			) {
				let f = 'YYYY:MM:DD:hh:mm:ss.SSS';
				let tz = 'UTC';
				if (jsonRowOld['sample' + '_time_zone'])
					tz = jsonRowOld['sample' + '_time_zone'];
				if (jsonRowOld['image' + '_time_zone'])
					tz = jsonRowOld['image' + '_time_zone'];
				if (jsonRowOld['plot' + '_time_zone'])
					tz = jsonRowOld['plot' + '_time_zone'];
				if (tz === 'CDT') tz = 'CST6CDT';
				if (tz === 'PDT') tz = 'PST8PDT';
				if (tz === '+8 GMT') tz = 'PRC';
				if (tz === '0') tz = 'UTC';
				...
				if (jsonRowOld['sample_date'])
					jsonRowOld['sample_date'] = moment
						.tz(jsonRowOld['sample_date'], f, tz)
						.tz('UTC')
						.format();
				if (jsonRowOld['image_date'])
					jsonRowOld['image_date'] = moment
						.tz(jsonRowOld['image_date'], f, tz)
						.tz('UTC')
						.format();
				if (jsonRowOld['plot_date'])
					jsonRowOld['plot_date'] = moment
						.tz(jsonRowOld['plot_date'], f, tz)
						.tz('UTC')
						.format();
			...
			if (jsonRowOld['specimen_scat'] === 'g')
				jsonRowOld['specimen_scat'] = 't';
			if (jsonRowOld['specimen_scat'] === 'b')
				jsonRowOld['specimen_scat'] = 'f';

			// Adopt sample inferred ages up to site ages.
			if (
				jsonRowOld['er_site_name'] &&
				jsonTableOld === 'pmag_samples' &&
				...
				if (jsonRowOld['sample_inferred_age'])
					siteRow['age'] = jsonRowOld['sample_inferred_age'];
				if (jsonRowOld['sample_inferred_age_sigma'])
					siteRow['age_sigma'] = jsonRowOld['sample_inferred_age_sigma'];
				if (jsonRowOld['sample_inferred_age_low'])
					siteRow['age_low'] = jsonRowOld['sample_inferred_age_low'];
				if (jsonRowOld['sample_inferred_age_high'])
					siteRow['age_high'] = jsonRowOld['sample_inferred_age_high'];
				if (jsonRowOld['sample_inferred_age_unit'])
					siteRow['age_unit'] = jsonRowOld['sample_inferred_age_unit'];
			...
			if (
				...
				let siteRow = { site: jsonRowOld['er_site_name'] };
				if (jsonRowOld['specimen_inferred_age'])
					siteRow['age'] = jsonRowOld['specimen_inferred_age'];
				if (jsonRowOld['specimen_inferred_age_sigma'])
					siteRow['age_sigma'] = jsonRowOld['specimen_inferred_age_sigma'];
				if (jsonRowOld['specimen_inferred_age_low'])
					siteRow['age_low'] = jsonRowOld['specimen_inferred_age_low'];
				if (jsonRowOld['specimen_inferred_age_high'])
					siteRow['age_high'] = jsonRowOld['specimen_inferred_age_high'];
				if (jsonRowOld['specimen_inferred_age_unit'])
					siteRow['age_unit'] = jsonRowOld['specimen_inferred_age_unit'];
				if (!this.jsonNew['sites']) this.jsonNew['sites'] = [];
				this.jsonNew['sites'].push(siteRow);
			...
			if (
				...
				if (jsonRowOld['average_age'])
					siteRow['age'] = jsonRowOld['average_age'];
				if (jsonRowOld['average_age_sigma'])
					siteRow['age_sigma'] = jsonRowOld['average_age_sigma'];
				if (jsonRowOld['average_age_low'])
					siteRow['age_low'] = jsonRowOld['average_age_low'];
				if (jsonRowOld['average_age_high'])
					siteRow['age_high'] = jsonRowOld['average_age_high'];
				if (jsonRowOld['average_age_unit'])
					siteRow['age_unit'] = jsonRowOld['average_age_unit'];
				if (!this.jsonNew['sites']) this.jsonNew['sites'] = [];
				this.jsonNew['sites'].push(siteRow);
			...
			if (
				jsonTableOld === 'pmag_sites' &&
				jsonRowOld['site_flag'] === undefined
			)
				jsonRowOld['site_flag'] = 'g';
			if (
				jsonTableOld === 'pmag_samples' &&
				jsonRowOld['sample_flag'] === undefined
			)
				jsonRowOld['sample_flag'] = 'g';
			if (
				jsonTableOld === 'pmag_specimens' &&
				jsonRowOld['specimen_flag'] === undefined
			)
				jsonRowOld['specimen_flag'] = 'g';
			if (
				jsonTableOld === 'rmag_anisotropy' &&
				jsonRowOld['anisotropy_flag'] === undefined
			)
				jsonRowOld['anisotropy_flag'] = 'g';
			if (
				jsonTableOld === 'rmag_hysteresis' &&
				jsonRowOld['hysteresis_flag'] === undefined
			)
				jsonRowOld['hysteresis_flag'] = 'g';
			if (
				jsonTableOld === 'rmag_remanence' &&
				jsonRowOld['remanence_flag'] === undefined
			)
				jsonRowOld['remanence_flag'] = 'g';
			if (
				jsonTableOld === 'rmag_susceptibility' &&
				jsonRowOld['susceptibility_flag'] === undefined
			)
				jsonRowOld['susceptibility_flag'] = 'g';

			// Insert the default result type
			if (
				jsonTableOld === 'pmag_results' &&
				jsonRowOld['data_type'] === undefined
			)
				jsonRowOld['data_type'] = 'i';
		}
		...
		for (let jsonColumnOld in jsonRowOld) {
			...
			if (this.versionNew === '3.0') {
				...
				if (jsonColumnOld === 'external_database_names') {
					...
					if (jsonRowOld['external_database_names'])
						dbNames = jsonRowOld['external_database_names']
							.replace(/(^:|:$)/g, '')
							.split(/:/);
					if (jsonRowOld['external_database_ids'])
						dbIDs = jsonRowOld['external_database_ids']
							.replace(/(^:|:$)/g, '')
							.split(/:/);

					for (let dbIdx in dbNames) {
						if (dbIDs.length <= dbIdx) dict.push(dbNames[dbIdx] + '[]');
						else dict.push(dbNames[dbIdx] + '[' + dbIDs[dbIdx] + ']');
					}
				...
				if (jsonTableOld === 'pmag_criteria') {
					...
					if (
						jsonColumnOld !== 'pmag_criteria_code' &&
						...
						if (jsonRowOld[jsonColumnOld] === -999) continue;
						...
						if (
							jsonRowOld['criteria_definition'] ||
							jsonRowOld['criteria_description']
						)
			...
			if (!this.modelOld['tables'][jsonTableOld]['columns'][jsonColumnOld]) {
				if (
					!this.undefinedTableColumnErrors[jsonTableOld + '.' + jsonColumnOld]
				)
					this._appendError(
						`Column "${jsonColumnOld}" in table "${jsonTableOld}" ` +
							`is not defined in MagIC Data Model version ${this.versionOld}.`
			...
			if (
				...
				if (
					!this.deletedTableColumnWarnings[jsonTableOld + '.' + jsonColumnOld]
				)
					this._appendWarning(
						`Column "${jsonColumnOld}" in table "${jsonTableOld}" ` +
			...
			for (let upgradeToTableAndColumnIdx in this.upgradeMap[jsonTableOld][
				...
				if (!joinTable || joinTable === jsonTableNew) {
					...
					if (this.versionNew === '3.0') {
						...
						if (
							jsonColumnNew.match(/^int_rel/) &&
							relativeIntensityNormalization
						)
							jsonColumnNew = jsonColumnNew.replace(
								/^int_rel/,
								'int_rel_' + relativeIntensityNormalization
							);

						// Combine descriptions without repetition.
						if (
							jsonColumnNew === 'description' &&
							tableRowsNew[jsonTableNew] &&
							tableRowsNew[jsonTableNew][0][jsonColumnNew] !== undefined
						) {
							if (
								tableRowsNew[jsonTableNew][0][jsonColumnNew].indexOf(
									jsonValueNew
								) != -1
							)
								jsonValueNew = tableRowsNew[jsonTableNew][0][jsonColumnNew];
							else if (
								jsonValueNew.indexOf(
						...
						if (
							(jsonColumnNew === 'lat_s' || jsonColumnNew === 'lon_w') &&
							tableRowsNew[jsonTableNew] &&
							tableRowsNew[jsonTableNew][0][jsonColumnNew] !== undefined
						) {
							jsonValueNew =
						...
						if (
							(jsonColumnNew === 'lat_n' || jsonColumnNew === 'lon_e') &&
							tableRowsNew[jsonTableNew] &&
							tableRowsNew[jsonTableNew][0][jsonColumnNew] !== undefined
						) {
							jsonValueNew =
					...
					if (
						this.modelNew['tables'][jsonTableNew]['columns'][jsonColumnNew]
							.type === 'List' ||
						this.modelNew['tables'][jsonTableNew]['columns'][jsonColumnNew]
							.type === 'Dictionary'
					) {
					...
					if (
						tableRowsNew[jsonTableNew][0][jsonColumnNew] !== undefined &&
						tableRowsNew[jsonTableNew][0][jsonColumnNew] !== jsonValueNew &&
						this.collisionErrors[`${jsonTableOld}.${jsonColumnOld}`] !=
							`${jsonTableNew}.${jsonColumnNew}`
					) {
						this._appendError(
							`MagIC Data Model version ${this.versionOld} column "${jsonColumnOld}" in table ` +
		...
		for (let table in tableRowsNew) {
			...
			if (this.versionNew === '3.0') {
				for (let rowIdx in tableRowsNew[table]) {
					...
					if (
						table === 'samples' &&
						tableRowsNew[table][rowIdx]['orientation_quality'] === undefined
					) {
						for (let column in tableRowsNew[table][rowIdx]) {
							if (
								this.modelNew['tables'][table]['columns'][column].group ===
								'Orientation'
							) {
						...
						for (let column in tableRowsNew[table][rowIdx]) {
							if (
								...
								if (tableRowsNew[table][rowIdx]['method_codes']) {
									if (
										!tableRowsNew[table][rowIdx]['method_codes'].match(
											/(^|(\s*)?:)(\s*)?DE-BF(L|P)/i
										)
									) {
										let methodCodes = tableRowsNew[table][rowIdx][
											'method_codes'
										]
											.replace(/(^:|:$)/g, '')
											.split(/:/);
										methodCodes.push('DE-BFL');
										tableRowsNew[table][rowIdx]['method_codes'] = _(methodCodes)
											.sortBy()
											.sortedUniq()
											.join(':');
									}
					...
					if (
						table === 'specimens' &&
						tableRowsNew[table][rowIdx]['int_corr'] === undefined
					) {
						for (let column in tableRowsNew[table][rowIdx]) {
							if (
								this.modelNew['tables'][table]['columns'][column].group ===
					...
					if (
						table === 'specimens' &&
						tableRowsNew[table][rowIdx]['int_scat'] === undefined
					) {
						for (let column in tableRowsNew[table][rowIdx]) {
							if (
								this.modelNew['tables'][table]['columns'][column].group ===
	...
	_mergePromise() {
		...
		return Promise.each(tables, (table) => {
			...
			if (table === 'measurements' || table === 'contribution') {
				rowProgressCounter += this.json[table].rows
					? this.json[table].rows.length
					: this.json[table].length;
			...
			this.json[table] = _.sortBy(
				this.json[table],
				this.mergeKeys[table].map((k) => {
			...
			const rowIdxChunks = _.chunk(
				_.range(this.json[table].length),
				this.nRowsBetweenProgressEvents
			);
			if (rowIdxChunks.length === 0) return Promise.resolve();
	...
	_mergeTableRow(table, rowIdx) {
		...
		const rowKeys = this.mergeKeys[table]
			? _.pick(this.json[table][rowIdx], this.mergeKeys[table])
			: this.json[table][rowIdx];
		...
		for (
			let rowCandidateIdx = rowIdx + 1;
			rowCandidateIdx < this.json[table].length;
			rowCandidateIdx++
		) {
			// Skip rows that are empty (e.g. ones that have already been merged).
			...
			if (
				_.isMatchWith(rowMerged, rowToMerge, (val1, val2, key) => {
				...
				if (
					rowMerged.description &&
					rowToMerge.description &&
					rowMerged.description !== rowToMerge.description
				) {
	...
	getUpgradeMap(model) {
		...
		for (let newTableName in model.tables) {
			//this gets the STRING name of the property into 'table'
			...
			for (let newColumnName in newTableObject.columns) {
				...
				if (prevColArray && prevColArray.length === 1) {
					...
					if (!upgradeMap[previousColTableName])
						upgradeMap[previousColTableName] = {};
					if (!upgradeMap[previousColTableName][previousColumnName])
						upgradeMap[previousColTableName][previousColumnName] = [];

					//TEST FOR TABLE/COLUMNS WITH NO CHANGES
					if (
						newColumnName == previousColumnName &&
						newTableName == previousColTableName
					) {
						upgradeMap[previousColTableName][previousColumnName].push({
							table: newTableName,
              ...
              upgradeMap[tmpPreviousColTableName][tmpPreviousColumnName].push([{table:newTableName, column:newColumnName}]);*/
						...
						upgradeMap[tmpPreviousColTableName][tmpPreviousColumnName].push({
							table: newTableName,
							column: newColumnName,
						});
					}
