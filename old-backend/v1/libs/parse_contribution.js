...
class Parser extends Runner {
	...
	parsePromise({
		text = undefined,
		nLinesBetweenProgressEvents = 1000,
		...
		return new Promise.each(
			...
			for (let jsonTable in this.json) {
				if (this.json[jsonTable].length === 0)
					this._appendWarning(
						`No data values were found in the ${jsonTable} table.`
					);
			}
		});
	}
	...
	_parseLine(line, format) {
		...
		// If this is the first line of a table, look for the table name.
			...
			if (tableDefinition.length < 2) {
				this._appendError(
					`Invalid table definition on line ${this.lineNumber}. Expected something like "tab[tab]measurements[new line]".`
				);
				this.skipTable = true;
			}

			// Clean leading and trailing whitespace from each part of the table definition.
			tableDefinition = tableDefinition.map((value) => {
				return value.trim();
			});
			...
			if (!tableDefinition[0].match(/^tab( delimited)?(\s|$)/i)) {
				this._appendError(
					`Invalid table definition column delimiter "${tableDefinition[0]}" on line ${this.lineNumber}. Expected "tab" or "tab delimited".`
				);
				this.skipTable = true;
			}
			...
			// Tab has been found, check for table name.
				...
				this._appendError(
					`No table name following tab delimiter on line ${this.lineNumber}.`
				);
				this.skipTable = true;
			}
		...
		// If this is the header line of a table, look for the column names.
			...
			if (_.findIndex(this.columns, '') !== -1) {
				this._appendError(
					`Empty column names are not allowed on line ${this.lineNumber}.`
				);
				this.skipTable = true;
			}
			...
			if (this.columns.length !== _.uniq(this.columns).length) {
				this._appendError(
					`Found duplicate column names on line ${this.lineNumber}.`
				);
				this.skipTable = true;
			}
			...
			if (
				format === 'magic' &&
				(this.table === 'measurements' || this.table === 'magic_measurements')
			)
		...
		// Otherwise, parse the row.
			...
			if (values.length > this.columns.length) {
				this._appendError(
					`More values found than columns on line ${this.lineNumber}: ${line}`
				);
				this.skipTable = true;
			}
			...
			// Append the row of values onto the table in the JSON.
				...
				else this.json[this.table].push(row);
			}
		}
	}
}
