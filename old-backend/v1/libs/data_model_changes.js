...
export default class extends Runner {
	...
	changes(model) {
		...
		console.log(
			'Outlining changes for data model: ' + this.dataModelVersionNumber
		);
		for (let currentTableName in dataModel.tables) {
			if (!hasOwnProperty.call(dataModel.tables[currentTableName], 'columns')) {
				this._appendError(
					`failed to find the columns list for table: ${currentTableName}`
				);
			}

			console.log(
				`--------------Ver ${this.dataModelVersionNumber} changes for table: ${currentTableName} ---------------`
			);
			for (let currentColumnName in dataModel.tables[currentTableName]
				.columns) {
				let prevColArray =
					dataModel.tables[currentTableName].columns[currentColumnName]
						.previous_columns;
				...
				if (prevColArray == undefined || prevColArray.length == 0) {
					if (this.dataModelVersionNumber != '2.0') {
						//All columns in 2.0 are considered "new"
						let insertedElement = {
						...
						_.set(
							this.modelChanges.inserted_columns,
							`tables[${currentTableName}].columns[${currentColumnName}]`,
							{}
						);
						console.log(
							`**** INSERTED columns: " ${currentColumnName}  Ver: ${this.dataModelVersionNumber}, Table: ${currentTableName}  ****`
						);
					}
				...
				if (!hasOwnProperty.call(prevColArray[0], 'table')) {
					this._appendError(
						`failed to find the previous table name for column ${currentColumnName} in table ${currentTableName}`
					);
					prevColIntegrity = false;
				}
				if (!hasOwnProperty.call(prevColArray[0], 'column')) {
					this._appendError(
						`failed to find the previous column name for column ${currentColumnName} in table ${currentTableName}`
					);
					prevColIntegrity = false;
				}
				if (prevColIntegrity === true) {
					...
					continue;
				} // The only scenario where this integrity is not needed is the deleted column, already taken care of

				//TEST FOR DELETED COLUMNS
				if (false) {
					//GGG PUTTING THIS OFF FOR A BIT
					let deletedElement = {
						table: currentTableName,
						column: currentColumnName,
					};
					...
					console.log(
						`**** DELETED column: " ${currentColumnName}  Ver: ${this.dataModelVersionNumber}, Table: ${currentTableName}  ****`
					);
				}

				//TEST FOR UNCHANGED COLUMNS AND COLUMN NAME CHANGES
				if (currentColumnName === previousColumnName) {
					//There is no change at all
					continue;
				} else if (prevColArray.length === 1) {
					//THERE IS A COLUMN NAME CHANGE
					this.processColumnNameChange(
						currentTableName,
						currentColumnName,
						previousColumnName
					);
				} else if (prevColArray.length > 1)
					this.processMergedColumns(
						prevColArray,
						currentTableName,
						currentColumnName
					);
	...
	processColumnNameChange(
		currentTableName,
		currentColumnName,
		previousColumnName
	) {
		console.log(
			`**** Column name change detected, Ver: ${this.dataModelVersionNumber}, Table: ${currentTableName}  ****`
		);
	...
	processMergedColumns(prevColArray, currentTableName, currentColumnName) {
		console.log(
			`**** MERGED column: " ${currentColumnName}  Ver: ${this.dataModelVersionNumber}, Table: ${currentTableName}  ****`
		);
		_.set(
			this.modelChanges.merged_columns,
			`tables[${currentTableName}].columns.${currentColumnName}`,
	...
	criticalModelValidation(model) {
		...
		if (!hasOwnProperty.call(model, 'tables')) {
			//console.log("Model: " + model);
			this._appendError(
				`the model argument ${model} has no "tables" property.`
			);
