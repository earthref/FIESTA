...
Infra:      Docker Compose locally; prod deploy story is open (deployment workflows removed 2026-09-07)
...
- **Multi-node isolation invariants**: contributions carry a `node` column; each node has its own Postgres schema, OpenSearch index, bucket prefix, and procrastinate queue; accounts are shared (one EarthRef login). A migration runs once per node schema — write Alembic migrations schema-agnostic (they run under a `schema_translate_map`). Legacy Meteor apps share the OpenSearch cluster with indices named plainly `magic`, `cdr`, … — `fiesta rebuild` deletes and recreates an index, so `FIESTA_INDEX_PREFIX` is mandatory on any shared cluster.
