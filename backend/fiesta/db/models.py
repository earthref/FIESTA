...
state machine. Contribution *data* lives in the storage bucket (canonical
files + manifests) and OpenSearch (denormalized search documents); both the
...
class Contribution(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    # Node slug (magic, cdr, ...) — multiple nodes can share one Postgres, so
    # every workflow row is scoped to the node it belongs to. Users are shared
    # (one EarthRef account works on every node).
    node: Mapped[str] = mapped_column(String(32), index=True)
    version: Mapped[int] = mapped_column(Integer, default=1)
    previous_id: Mapped[int | None] = mapped_column(ForeignKey("contributions.id"))
    ...
    status: Mapped[ContributionStatus] = mapped_column(
        Enum(ContributionStatus, values_callable=lambda e: [m.value for m in e]),
...
class ValidationResult(Base):
    ...
    contribution_id: Mapped[int] = mapped_column(
        ForeignKey("contributions.id", ondelete="CASCADE"), index=True
    )
